...
class PayPay:
    ...
    async def link_receive(
        self, url: str, passcode: str = None, link_info: dict = None
    ) -> dict:
        if not self.access_token:
            raise PayPayLoginError("まずはログインしてください")
    ...
    async def create_link(
        ...
        amount: int,
        passcode: str = None,
        pochibukuro: bool = False,
        theme: str = "default-sendmoney",
    ):
        if not self.access_token:
            raise PayPayLoginError("まずはログインしてください")
    ...
    async def search_p2puser(
        ...
        if is_global:
            ...
            name = p2puser["payload"]["friendsAndCandidatesSearchResults"]["friends"][
                order
            ]["displayName"]
            icon = p2puser["payload"]["friendsAndCandidatesSearchResults"]["friends"][
                order
            ]["photoUrl"]
            external_id = p2puser["payload"]["friendsAndCandidatesSearchResults"][
                "friends"
            ][order]["externalId"]

        return P2PUser(name, icon, external_id, p2puser)

    async def initialize_chatroom(self, external_id: str):
        if not self.access_token:
            raise PayPayLoginError("まずはログインしてください")
    ...
    async def alive(self) -> None:
        ...
        await self.session.post(
            "https://app4.paypay.ne.jp/bff/v3/getHomeDisplayInfo?payPayLang=ja",
            headers=self.headers,
            json={
